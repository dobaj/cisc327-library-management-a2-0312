...
def get_book_by_author(author: str) -> List[Dict]:
    """Get books by author."""
    conn = get_db_connection()
    books = conn.execute('SELECT * FROM books b WHERE LOWER( b.author ) LIKE ?', ("%"+author.lower()+"%",)).fetchall()
    conn.close()
    return [dict(book) for book in books]
...
def get_book_by_title(title: str) -> List[Dict]:
    """Get books by title."""
    conn = get_db_connection()
    books = conn.execute('SELECT * FROM books b WHERE LOWER( b.title ) LIKE ?', ("%"+title.lower()+"%",)).fetchall()
    conn.close()
    return [dict(book) for book in books]
...
def get_patron_prev_borrowed_books(patron_id: str) -> List[Dict]:
    """Get previously borrowed books for a patron."""
    conn = get_db_connection()
    records = conn.execute('''
        SELECT br.*, b.title, b.author 
        FROM borrow_records br 
